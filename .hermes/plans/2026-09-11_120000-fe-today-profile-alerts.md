...
- `frontend/src/styles/theme.css` (587 lines) already has tokens, the app shell, BottomNav, TopHeader, risk dots, metric/list styles, alert rows, hospital rows, note block. We'll add styles for the new hero card and the profile form. **No hardcoded colors** — every color must reference a CSS variable already in `theme.css` (or extend `theme.css` to add the var).
